    ...
    const HENDERSON_NEEDS = [
      { id: 1, name: "呼吸", icon: "fa-lungs", keywords: ["呼吸", "SpO2", "咳", "痰", "喘鳴", "息切れ", "チアノーゼ", "酸素", "息苦し", "PaO2", "呼吸数", "呼吸音", "動脈血ガス", "胸部X線", "胸郭", "呼吸補助筋", "起座呼吸", "wheezes", "発汗", "気道", "分泌物", "狭窄", "アレルギー", "喫煙", "人工呼吸器", "酸素マスク", "肥満", "発熱", "疼痛", "副作用", "安楽な姿勢", "体位", "枕", "湿度", "臭気", "吸入器", "在宅酸素療法", "排痰", "咳嗽", "血圧", "脈拍", "頻脈", "徐脈", "不整脈", "動悸", "浮腫", "末梢冷感", "循環動態", "心拍数", "心電図", "胸痛", "ショック", "麻酔", "全身麻酔", "麻酔導入", "麻酔覚醒", "気管挿管", "抜管", "麻酔科", "弾性ストッキング", "無気肺", "有効咳嗽", "術後肺炎", "深部静脈血栓症", "DVT", "肺血栓塞栓症", "PTE",
        ...
        // 「顔色やや蒼白、息遣いは浅い。」がタグ未設定になっていた）。
        // 「一秒率」「肺活量」は呼吸機能検査（％肺活量・一秒率）の項目名で、そのまま呼吸機能の
        // 検査データにあたるため追加する。
        "顔色", "蒼白", "息遣い", "一秒率", "肺活量",
        // 「PCA」（自己調節鎮痛法）・「アセリオ」「レペタン」（術後の疼痛時指示でよく使われる
        // 鎮痛薬名）は、いずれも疼痛管理に直結する語だが、「【疼痛時指示】①PCA、②PCA
        // 無効時アセリオ1000mg、③レペタン静注0.1mg」のような「、」列挙の行が分割された際、
        // 先頭の「疼痛時指示」の文脈を失った2件目以降の断片がタグ未設定のまま残っていた
        // （利用者からのアップロード文書で発覚）。
        "PCA", "アセリオ", "レペタン",
        // 大腿骨近位部骨折の周術期資料より、DVT/PTE予防に使う装具・徴候名を追加する。
        // 「深部静脈血栓症」「DVT」「肺血栓塞栓症」「PTE」は既存だが、装具名（フットポンプ・
        // 間欠的空気圧迫装置・IPC）や「肺塞栓症」（「血栓」を含まない表記）、DVTの代表的な
        // 所見（Homans徴候・腓腹部の把握痛）は未登録だったため追加する。
        "肺塞栓症", "間欠的空気圧迫", "間歇的空気圧迫", "フットポンプ", "IPC", "Homans徴候", "把握痛",
        // 肺炎の周術期・治療資料より、酸素療法デバイスの種類・CO2ナルコーシス・排痰援助・
      ...
      { id: 2, name: "食事", icon: "fa-utensils", keywords: ["食事", "食欲", "摂取量", "水分量", "嚥下", "嘔吐", "悪心", "吐気", "むせ", "体重", "栄養", "飲水", "Alb", "TP", "食事量", "咀嚼", "嚥下機能", "栄養状態", "BMI", "食習慣", "間食", "外食", "宗教的習慣", "食事療法", "必要エネルギー", "透析", "造影剤", "味覚", "化学療法", "身長", "ローレル指数", "カウプ指数", "頭皮", "毛髪", "免疫", "義歯", "自助具", "口腔粘膜", "胃がん", "胃癌", "胃切除", "胃全摘", "噴門", "幽門", "ダンピング症候群", "逆流性食道炎", "分割食", "小胃症状",
        ...
        // アップロード文書で発覚）。
        "食生活",
        // 肺炎（誤嚥性肺炎）の資料より、誤嚥・嚥下機能評価・食事指導に関する語を追加する
        // （利用者からのアップロード資料により判明）。
        "誤嚥", "不顕性誤嚥", "湿性嗄声", "嚥下調整食", "とろみ", "とろみ調整食品", "RSST", "VF", "VE", "嚥下造影", "嚥下内視鏡"] },
      { id: 3, name: "排泄", icon: "fa-toilet", keywords: ["排尿", "排便", "下痢", "便秘", "失禁", "導尿", "残尿", "BUN", "Cre", "eGFR", "おしっこ", "下血", "血尿", "ストーマ", "導尿カテーテル", "ドレーン排液", "排泄", "緩下剤", "下剤", "浣腸", "おむつ", "肛門", "臀部", "腹部", "夜間排尿", "利尿剤", "尿道カテーテル", "ウロストミー", "混濁", "出血", "滲出液", "体臭", "腸蠕動", "排ガス", "イレウス", "腹部膨満",
        // 「カテーテル」（膀胱留置カテーテル等）「尿量」「尿検査」「尿糖」「尿蛋白」は、いずれも
        // 排泄（排尿）に直結する語だが、既存の「尿道カテーテル」等の限定的な表記だけでは
        // 「膀胱留置カテーテル」のように前に付く語が違うと一致せず、タグ未設定のまま残って
        // いた（利用者からのアップロード文書で発覚）。
        "カテーテル", "尿量", "尿検査", "尿糖", "尿蛋白",
      ...
      { id: 4, name: "姿勢", icon: "fa-person-walking", keywords: ["歩行", "移乗", "立位", "坐位", "麻痺", "可動域", "拘縮", "転倒", "ベッド上安静", "体位変換", "移動", "運動機能", "発赤", "褥瘡", "クッション", "自助具", "ベッド柵", "輸液ライン", "滑りにくい靴", "筋力", "視覚", "聴覚", "感覚機能", "循環機能", "麻酔", "全身麻酔", "麻酔覚醒", "麻酔後回復室", "術後体動制限", "麻酔科", "眼鏡", "コンタクトレンズ", "感覚障害", "デスクワーク", "早期離床", "ファウラー位", "端坐位", "体位ドレナージ", "側臥位", "スポーツ", "運動能力", "テニス", "ゴルフ", "サッカー", "野球", "水泳", "ジョギング", "ランニング",
        ...
        // 「安静」「安静度」は「ベッド上安静」の一部表記だけでは「術後床上安静」のように
        // 前に付く語が違うと一致せず、活動制限の指示がタグ未設定のまま残っていた
        // （いずれも利用者からのアップロード文書で発覚）。
        "ADL", "自立", "安静", "安静度",
        // 大腿骨近位部骨折（人工骨頭置換術BHA・人工股関節全置換術THA等）の周術期資料より、
        // 脱臼予防（禁忌肢位）・段階的荷重・リハビリテーションに関する語を追加する。
        // いずれも動作・姿勢保持そのものに直結するが、既存のキーワードでは拾えなかった
        // （利用者からのアップロード資料により判明）。
        "脱臼", "禁忌肢位", "外転枕", "良肢位", "内転", "内旋", "外旋",
      ...
      { id: 9, name: "環境", icon: "fa-shield-halved", keywords: ["コール", "ベッド柵", "点滴ルート", "チューブ抜去", "転落防止", "身体抑制", "安全管理", "転倒", "転落", "事故", "交通事故", "身体損傷", "暴力", "感染症", "治安", "経済力", "標準予防策", "感染", "院内感染", "接触感染", "飛沫感染", "空気感染", "隔離", "手指衛生", "個人防護具", "多剤耐性菌", "MRSA", "易感染状態", "創部", "縫合", "ドレーン", "創部感染", "創部観察", "ガーゼ交換", "滲出液", "離開", "縫合不全", "膵液漏", "腹膜炎", "自己抜去", "CAUTI",
        ...
        "人間関係",
        // 「チューブ」「硬膜外」（硬膜外チューブ・硬膜外PCA等）は、既存の「チューブ抜去」
        // だけでは前に付く語（硬膜外・尿道等）が違う場合に一致せず、留置物の管理に関する
        // 情報がタグ未設定のまま残っていた。「点滴」は「点滴ルート」の言い換えとしてよく
        // 使われる短い表記だが未登録だった。「アミラーゼ」はドレーン排液の膵液漏監視に
        // 使う検査値（NotebookLM基準ノートの「ドレーン管理」参照）のため、環境（ドレーン管理）
        // に含める。「HBV」「HCV」「梅毒」は感染症の検査項目名で、既存の「感染症」キーワード
        // 自体は見出し語として本文から切り出されてしまうため、これら個別の検査名も直接
        // 登録する（いずれも利用者からのアップロード文書で発覚）。
        "チューブ", "硬膜外", "点滴", "アミラーゼ", "HBV", "HCV", "梅毒",
        // 大腿骨近位部骨折の周術期資料より、手術部位感染（SSI）・骨接合術後の固定失敗徴候
        // （カットアウト）を追加する（利用者からのアップロード資料により判明）。
        "SSI", "手術部位感染", "カットアウト",
        // 肺炎の資料より、人工呼吸器関連肺炎（VAP）・院内肺炎(HAP)等の院内感染管理に
        // 関する語を追加する（利用者からのアップロード資料により判明）。
        "VAP", "人工呼吸器関連肺炎"] },
      { id: 10, name: "コミュニケーション", icon: "fa-comments", keywords: ["発話", "会話", "聞こえ", "視力", "難聴", "表出", "失語", "認知", "不安", "訴え", "話す", "語る", "コミュニケーション", "構音", "意思伝達", "手話", "識字", "聴覚", "自尊感情", "自己否定", "自傷", "価値観", "ボディイメージ", "依存", "せん妄", "不穏", "見当識", "リアリティ・オリエンテーション",
        // 大腿骨近位部骨折の周術期資料より、せん妄のタイプ（過活動型・低活動型）を追加する。
        "過活動型", "低活動型"] },
      ...
      { id: 14, name: "学び", icon: "fa-graduation-cap", keywords: ["病識", "理解度", "指導", "服薬指導", "質問", "知りたい", "学習", "説明理解", "学歴", "学習困難", "教材", "健康習慣", "血糖測定", "飲酒",
        // 大腿骨近位部骨折の周術期資料より、骨粗鬆症治療・二次骨折予防（骨折リエゾンサービス）の
        // 継続的な自己管理・健康教育に関する語を追加する。
        "骨粗鬆症", "二次骨折予防", "骨折リエゾンサービス", "FLS", "ビスホスホネート",
        // 肺炎の退院支援資料より、嚥下リハビリテーション・ワクチン接種等の継続的な自己管理・
        // 健康教育に関する語を追加する（利用者からのアップロード資料により判明）。
        "嚥下リハビリテーション", "肺炎球菌ワクチン", "PPSV23", "インフルエンザワクチン", "ワクチン接種"] }
    ];
...
【大腿骨近位部骨折（頸部骨折・転子部骨折／人工骨頭置換術・骨接合術）周術期看護 判断基準】
（出典：利用者アップロード資料「大腿骨近位部骨折の解剖・基礎知識・周術期看護」を統合。高齢者の骨折の中でも特に多く、受傷前からの併存症・生活障害を抱えるケースが多いため、骨折部自体の観察に加え術前後の臥床・侵襲に伴う全身合併症の早期発見が重要）

■ 病態・原因・症状
大腿骨近位部骨折は、股関節を構成する大腿骨近位端に発生する骨折で、関節包内骨折の大腿骨頸部骨折（血流が乏しく骨頭壊死・偽関節のリスクが高い）と関節包外骨折の大腿骨転子部骨折（血流は豊富で骨癒合しやすいが出血量が多く不安定型になりやすい）に大別される。高齢者の低エネルギー外傷（転倒等、全受傷原因の約74%）による骨粗鬆症を基盤とした脆弱性骨折が大半。症状は股関節・鼠径部や大転子周囲の激しい疼痛、患肢の短縮・外旋変形、腫脹・皮下出血、歩行不能。頸部骨折はGarden分類（非転位型Ⅰ・Ⅱ→骨接合術／転位型Ⅲ・Ⅳ→人工骨頭置換術BHA）、転子部骨折はEvans分類・AO/OTA分類（安定型・不安定型）で治療方針が決まる。全身状態が許せば受傷後48時間以内の早期手術が推奨され、保存療法は長期臥床による肺炎・DVT・褥瘡・認知機能低下等の重篤な合併症リスクが高いため原則手術療法が選択される。

■ 周術期看護の展開（術前・術中・術直後・術後1日目・術後2日目以降）
術前：疼痛コントロール（NSAIDs・神経ブロック等）、良肢位保持（外転枕で骨折部の転位・周囲組織損傷を防止）、皮膚状態・腫脹観察（褥瘡好発）、抗血栓薬の休薬・継続判断（休薬延長はSSI・死亡率を高めるため48時間以内の早期手術と天秤にかける）、呼吸機能訓練・DVT予防（足関節ポンプ運動）・せん妄予防（日中離床、見当識への働きかけ）を行う。術中：循環動態の連続モニタリング、セメント使用BHAでのBone Cement Implantation Syndrome（一過性の急激な血圧低下・低酸素血症）への注意、出血量把握、体位・皮膚受圧部（腓骨頭等）の保護。術直後：バイタルサイン・意識レベル、創部・ドレーン排液（血性→淡血性の推移）、患肢の末梢循環・神経（足趾の色調・冷感・CRT・足背動脈・足趾運動・しびれ）、股関節脱臼の有無（患肢の突発的な外旋・短縮変形はBHA/THA脱臼徴候）を観察する。術後1日目：疼痛再評価、離床開始（端坐位・車椅子移乗・起立性低血圧の確認）、DVT/PTE（下腿腫脹・Homans徴候、突然の呼吸苦・胸痛）、術後せん妄、周術期貧血（Hb低下、顔面蒼白・倦怠感・立位時のふらつき）を観察する。術後2日目以降：荷重状態・リハビリ進捗、栄養状態（Alb・食事摂取量）、尿路管理（尿道カテーテル早期抜去）、手術部位感染（SSI：発赤・熱感・腫脹・膿性排液・数日後の発熱）、骨接合術後のカットアウト徴候（転子部骨折。突然の激痛・荷重不能）、遅発性脱臼（BHA/THA。禁忌肢位逸脱）を観察する。

■ ドレーン管理（関節腔内留置）
目的は血腫形成予防・腫脹に伴う疼痛軽減・創部感染予防・創傷治癒促進。術直後は30分〜1時間ごと、翌日以降は2時間ごとに排液の性状・量を測定・記録する。術直後は血性で100〜200g/時間程度、経過とともに淡血性→漿液性へ変化し翌日には10〜20g/時間程度へ減少するのが正常経過。排液バッグは創部・膀胱より低位に保ち逆行性感染を防止し、ルートの屈曲・ねじれ・閉塞（ミルキングで解除）や刺入部固定の状態を点検する。排液量が減少し性状が漿液性になった時点（通常術後1〜2日／約36時間以内）で抜去される。

■ DVT（深部静脈血栓症）・肺塞栓症（PE/PTE）予防
受傷後の不動化・手術侵襲による静脈血流停滞・血管内皮損傷・血液凝固能不全でDVTが形成され、血栓が肺動脈を塞ぐとPE/PTEを引き起こす。DVTは下腿の腫脹（左右差）・腓腹部の把握痛（Homans徴候）・熱感、PEは突然の胸痛・呼吸困難・SpO2低下・血圧低下を呈する（発症時は直ちに歩行中止・ベッド上安静・医師へ緊急要請）。看護は術後早期からの足関節ポンプ運動と早期離床、間欠的空気圧迫装置（フットポンプ／IPC。手術室入室〜離床開始まで装着）、弾性ストッキング（術前から継続、フットポンプ併用時は爪床の色調・皮膚温・しびれを観察）、抗凝固薬使用時の出血傾向（創部出血・血尿）の観察を行う。

■ 疼痛管理
創部痛（安静時痛）と体動時痛（運動時痛）に分類され、疼痛は交感神経刺激による血圧上昇・心負荷、無気肺・肺炎、離床遅延、術後せん妄の誘発因子となる。消炎鎮痛薬の定時投与、オピオイドのレスキュー、大腿神経ブロック（FNB）・腸骨筋膜下ブロック（FICB）による鎮痛が行われる。看護は良肢位（膝下・足間にクッション・外転枕）の保持、術直後急性期の患部クーリング、離床・リハビリ前の予防的鎮痛（事前投与）を行う。

■ 脱臼予防（人工骨頭置換術BHA・人工股関節全置換術THA施行時の禁忌肢位）
筋肉・関節包が回復していない時期に特定方向へ股関節を動かすと人工骨頭が寛骨臼から脱臼する。後方アプローチ（一般的）の禁忌肢位は「屈曲90度以上＋内転（脚を交差）＋内旋（つま先を内側）」の複合動作、前方アプローチは「過度の伸展＋外旋」。看護は体位変換・就寝時に両脚間へ外転枕・クッションを挟む、車椅子移乗・座位では高めの椅子を選択し浅く腰掛けない、和式トイレを避け洋式トイレ・補高便座を用いる、床の物を拾う・靴下着脱・爪切りにはソックスエイド等の自助具を活用するよう指導する。

■ 術後せん妄
準備要因（高齢・認知症既往・施設入所歴・低栄養）に、侵襲・疼痛・低酸素血症・電解質異常・環境変化・夜間不眠・身体拘束等の誘因が加わって発症する。過活動型（興奮・不穏・ナースコール不適切使用・抜管抜針）と低活動型（活気低下・無関心。見落とされやすい）がある。看護は多職種による予防的介入、疼痛管理・酸素投与・電解質バランスの維持、尿道カテーテル・ドレーンの早期整理による身体拘束の最小化、日中の離床・日光と夜間の消灯によるメリハリ、カレンダー・時計・声かけによる見当識への働きかけを行い、発症時は危険行動を否定せず傾聴・安全確保する。

■ 尿道カテーテル管理
挿入・管理時は無菌操作、閉鎖系・逆流防止（採尿バッグは膀胱より低位）の維持、交差感染防止（個別容器使用）を徹底する。尿路感染症（CAUTI）の兆候（尿の濁り・浮遊物・異臭・発熱・下腹部不快感）を観察し、術後3日程度以内の短期間使用が推奨され、自力での排尿運動・車椅子離床が可能となった段階で速やかに抜去可否をアセスメントする。

■ リハビリテーションと退院支援（多職種連携・二次骨折予防）
術後早期からの離床・段階的荷重（足関節ポンプ運動・大腿四頭筋セッティング→端坐位・起立訓練→平行棒内歩行〜歩行器・T字杖歩行）が廃用症候群・肺炎・DVT・認知機能低下を防ぐ。整形外科医・老年病医・看護師・理学療法士・作業療法士・社会福祉士・管理栄養士・薬剤師等の多職種チーム診療（Orthogeriatric co-management）と地域連携パスにより在宅復帰を支援する。退院前の家屋評価（手すり設置・段差解消・補高便座・介護ベッド・自助具選定）、骨折リエゾンサービス（FLS）による骨粗鬆症治療薬（ビスホスホネート製剤等）の開始・継続フォローアップで二次骨折を予防する。

■ ヘンダーソン14項目との主な対応（AI分類時の初期タグ提案に利用）
1.呼吸＝DVT/PTE・循環動態・麻酔管理・フットポンプ/弾性ストッキング／2.食事＝低栄養（Alb・BMI。せん妄リスク要因）／3.排泄＝尿道カテーテル・早期抜去／4.姿勢＝良肢位・禁忌肢位・脱臼予防・早期離床・段階的荷重・リハビリテーション／9.環境＝創部感染(SSI)・ドレーン管理・カットアウト・二次骨折予防／10.コミュニケーション＝術後せん妄（過活動型・低活動型）／14.学び＝骨粗鬆症治療の理解・自己管理、二次骨折予防（FLS）。あくまで初期提案であり、実際の記録内容や学習結果（利用者の修正）があればそちらを優先する。

【肺炎（市中肺炎CAP・院内肺炎HAP・医療介護関連肺炎NHCAP・誤嚥性肺炎）看護 判断基準】
（出典：利用者アップロード資料「呼吸器系の解剖生理・肺炎の基礎知識・治療管理・退院支援」を統合。高齢者では発熱や強い咳・痰といった典型症状が出にくく発見が遅れやすいこと、誤嚥性肺炎の関与が大きいことから、呼吸管理に加えて嚥下機能・口腔ケア・退院後の再発予防指導が特に重要）

■ 病態・分類
肺炎は肺実質（肺胞領域）に生じる急性の感染性炎症で、炎症により肺胞内に滲出液・白血球が溜まりガス交換が阻害される。罹患場所により、基礎疾患のない人に発症し耐性菌リスクが低く予後良好な市中肺炎（CAP）、入院48時間以降に新しく発症し耐性菌リスク・死亡率が高い院内肺炎（HAP。人工呼吸器装着48時間以降に発症する人工呼吸器関連肺炎VAPを含む）、療養・介護施設入所者や退院後90日以内の患者・要介護高齢者等に発症し耐性菌・誤嚥の要素を持つ医療・介護関連肺炎（NHCAP。CAPとHAPの中間）に分類される。原因微生物により、肺炎球菌等の細菌性肺炎（細胞壁を持ちβ-ラクタム系抗菌薬が有効）、肺炎マイコプラズマ等の非定型肺炎（細胞壁を持たずβ-ラクタム系無効、マクロライド系・ニューキノロン系が有効）、インフルエンザウイルス等のウイルス性肺炎に分類される。誤嚥性肺炎は嚥下機能低下・咳嗽反射障害・意識障害等の誤嚥リスクを有する宿主に、不顕性誤嚥（むせを伴わない誤嚥）も含めて口腔内常在菌（レンサ球菌・嫌気性菌等）が下気道・肺胞へ落ち込み発症する病態メカニズムによる分類で、CAP/HAP/NHCAPのいずれにも重なって存在しうる。高齢者肺炎の70%以上に誤嚥が関与するとされ、抗菌薬治療だけでなく口腔ケア・食事姿勢・食形態調整・嚥下・呼吸リハビリテーションを含めた包括的管理が重要。

■ 症状（高齢者の非典型症状に注意）
局所症状は咳嗽（細菌性は湿性咳嗽、非定型・ウイルス性は乾性咳嗽が多い）、黄色〜緑色の粘稠痰・膿性痰、呼吸困難・息切れ（呼吸数増加・SpO2低下）、深呼吸・咳嗽時の胸痛（胸膜まで炎症が及んだ場合）。全身症状は発熱・悪寒、全身倦怠感・頭痛・筋肉痛。高齢者・誤嚥性肺炎では免疫応答の低下から典型的な呼吸器症状（高熱・強い咳や痰）が出にくく、微熱または無熱で経過することが多い。主な初期徴候は「なんとなく元気がない（活気低下）」「食事をとらない（食欲不振）」「普段より反応が鈍い・意識がぼんやりしている・せん妄」「よくつまずく・ふらつく・体動不能」「のどがゴロゴロ鳴る・失禁」で、周囲が日常の微細な変化に気づかないと発見が遅れ重症化するリスクが高い。

■ 検査
血液検査（WBC：細菌性肺炎では好中球優位の白血球増加、CRP：炎症の強さを反映し診断・治療効果判定に用いる）、胸部X線検査（確定診断の第一選択。浸潤影・気管支透亮像の有無を確認）、胸部CT検査（心臓・横隔膜の陰に隠れて写りにくい背側病変の検出や、非定型肺炎のすりガラス影・細気管支炎像との鑑別に高精度）、喀痰培養検査（グラム染色・培養で起炎菌と薬剤感受性を同定。必要に応じ血液培養2セット・尿中抗原検査を併用）、動脈血ガス分析（PaO2・PaCO2・pHから呼吸不全の程度・低酸素血症の重症度・CO2ナルコーシスの有無等を評価）。

■ 治療
抗菌薬投与が基本で、原因菌確定前は推定菌を広くカバーするエンピリック療法を開始し、判明後は感受性のある狭域抗菌薬へ変更（de-escalation）する。細菌性肺炎はペニシリン系・セフェム系・カルバペネム系等、非定型肺炎はマクロライド系・ニューキノロン系・テトラサイクリン系を使用し、状態改善に応じて注射薬から内服薬へのスイッチ療法を行う。解熱鎮痛薬・去痰薬を対症療法として併用する。低酸素血症には酸素療法（鼻カニューレ・酸素マスク等）を行い、重症の呼吸不全例では人工呼吸器・NPPV（非侵襲的陽圧換気）・ECMOを検討する。発熱・頻呼吸による不感蒸泄増大や経口摂取低下には輸液で水分・電解質・栄養バランスを維持するが、過剰輸液による肺水腫に注意する。

■ 酸素療法デバイスの管理
低流量システム（不足分を室内気で補う）：鼻カニューレ（軽度の低酸素血症、1〜6L/分・実際は3L/分程度まで推奨、FiO2約24〜44%、6L/分超は鼻粘膜刺激のため不適、口呼吸には不向き）、簡易酸素マスク（中等度の低酸素血症、5〜8L/分必須、FiO2約40〜60%、CO2再吸入防止のため5L/分未満にする場合は鼻カニューレへ変更）、リザーバー付き酸素マスク（重度の低酸素血症・高濃度酸素60%以上、6〜15L/分・通常10〜15L/分、FiO2約60〜90%以上、バッグが虚脱しないよう流量維持が必須）。高流量システム（患者の最大吸気流量以上を供給し一定のFiO2を維持）：ベンチュリマスク（ベンチュリ効果で換気パターンに左右されず安定したFiO2 24〜50%を維持でき、Ⅱ型呼吸不全（COPD合併等）に最適）、HFNC（加温加湿した高流量酸素・最大60L/分程度、FiO2 21〜100%を独立設定可能）、ネブライザー付き酸素吸入器（安定したFiO2供給と十分な加湿を同時に行う）。

■ CO2ナルコーシス
COPD等の慢性Ⅱ型呼吸不全（慢性高二酸化炭素血症）患者は、PaCO2上昇に対する呼吸中枢の感度が低下し主に低酸素（PaO2低下）を感知して呼吸を促進しているため、高濃度酸素を投与するとPaO2上昇により呼吸駆動が抑制され、肺胞低換気となってCO2がさらに蓄積し中枢神経症状を引き起こす。3主症状は意識障害（初期の頭痛・発汗・頻脈・呼吸促迫から進行すると傾眠〜昏睡）、高度の呼吸性アシドーシス（pH低下・PaCO2著明上昇）、自発呼吸の減弱・抑制。防止・管理のため、既往歴を確認し高リスク患者にはベンチュリマスク等で低濃度（24%等）から酸素を開始してターゲットSpO2 88〜92%を維持管理し、意識レベル・動脈血ガス分析値を厳重に監視する。

■ 排痰援助（気道クリアランスの改善）
分泌物（粘稠痰）の増加は気道抵抗の増大・無気肺・低酸素血症を引き起こすため排痰ケアが不可欠。体位ドレナージ（聴診・画像所見で痰が貯留する病変部位を上にした体位を保持し、重力で末梢の痰を中枢気道へ移動・集約させる）、スクイージング（呼気時に胸郭を手で圧迫し胸郭運動・呼気流速を高めて末梢気道の痰を中枢へ押し出す用手排痰法。体位ドレナージやハフィング＝声を出さず「ハッ！ハッ！」と息を吐く有効な咳嗽と併用すると効果的）、ネブライザー吸入（薬剤・生理食塩液をエアロゾル化して気道粘膜を加湿・潤滑し痰を柔らかくする。排痰の10〜20分前が効果的、機器は細菌が繁殖しやすいため使用後の洗浄・乾燥管理を徹底）、気道吸引（自力排痰困難時に実施。定期的・一律な吸引は避け副雑音・呼吸音減弱・SpO2低下等から必要性を判断、吸引圧は気道粘膜損傷・出血・低酸素血症を避けるため200mmHg以下に制限、無菌操作・短時間で実施。合併症は気道粘膜損傷・出血、低酸素血症、不整脈、血圧変動、無気肺、気管支攣縮）。

■ 抗菌薬投与管理
治療効果を高め耐性菌発生を防ぐため決められた投与時刻・間隔を厳守する（カルバペネム系・ペニシリン系は直前溶解が必要）。バンコマイシン等はRed man症候群（急速投与によるヒスタミン遊離に伴う顔面・上半身の潮紅や血圧低下）予防のため60分以上かけて点滴静注する。有効域と中毒域が近く腎毒性リスクが高い抗菌薬（バンコマイシン、テイコプラニン、アミノグリコシド系等）ではTDM（薬物血中濃度モニタリング）を実施し、トラフ値・ピーク値の測定と腎機能（Cr・eGFR・Ccr）に応じた投与量調整を行う。主な副作用はアナフィラキシー（投与直後〜数十分以内の発疹・蕁麻疹・掻痒感・喘鳴・呼吸困難・血圧低下、異常時は直ちに点滴中止）、抗菌薬関連下痢症・CD腸炎（水様便・頻回下痢・腹痛・発熱）、器官毒性（腎障害・肝障害。Cr, BUN, AST, ALT・尿量の定期確認）。

■ ヘンダーソン14項目との主な対応（AI分類時の初期タグ提案に利用）
1.呼吸＝頻呼吸・SpO2低下・副雑音（ラ音・コースクラックル・ロンカイ）・酸素療法デバイス管理・CO2ナルコーシス・排痰援助（体位ドレナージ・スクイージング・ハフィング・気道吸引）／2.食事＝食欲不振・嚥下機能低下・誤嚥（不顕性誤嚥含む）・食事中のむせ・湿性嗄声・脱水／5.睡眠＝夜間の咳嗽による中途覚醒・呼吸困難による睡眠障害／7.体温＝発熱・悪寒／8.清潔＝発汗による清潔不良・口腔ケア（誤嚥性肺炎予防の最重要策）／9.環境＝院内感染・多剤耐性菌・人工呼吸器関連肺炎(VAP)／14.学び＝嚥下リハビリテーション・食事指導（とろみ・嚥下調整食）・ワクチン接種・口腔ケアの自己管理理解。あくまで初期提案であり、実際の記録内容や学習結果（利用者の修正）があればそちらを優先する。

■ 退院支援（誤嚥性肺炎の再発予防）
嚥下機能の評価：RSST（反復唾液嚥下テスト。30秒間の唾液嚥下回数を測定）等のスクリーニングと、VF（嚥下造影検査）・VE（嚥下内視鏡検査）による精密評価（ゴールドスタンダード）。嚥下機能は日内変動し不顕性誤嚥も存在するため異常なしでも日常の経過観察を怠らない。摂食・嚥下リハビリテーション：医師・言語聴覚士（ST）・看護師の連携で間接訓練（発声・器官運動・頸部ストレッチ・嚥下筋トレーニング）と直接訓練（食形態を工夫した実際の摂食訓練）を行う。食事指導：嚥下機能に応じた嚥下調整食（ゼリー食・ムース食・ペースト/ミキサー食・きざみ食等）の選択、とろみ調整食品による粘度調整（薄い・中間・濃いとろみ）、食事姿勢（座位はあごを引く前傾姿勢、ベッド上は90度ギャッジアップまたは30〜60度＋クッションで支持）、食後30分〜1時間の座位保持による逆流・不顕性誤嚥の予防。ワクチン接種の啓発：65歳以上・高リスク者への肺炎球菌ワクチン（PPSV23）、年1回のインフルエンザワクチン（二次性細菌性肺炎を予防）、両者の併用接種による相加的な入院・死亡率低下効果。継続的な口腔ケア：誤嚥性肺炎の主な原因菌である口腔内常在菌を減らすため、歯ブラシ・義歯ブラシ・粘膜ブラシで歯・義歯・舌苔に固着したバイオフィルムを物理的に除去（うがいだけでは除去できない）、うがい困難者にはスポンジブラシや吸引での拭き取り・排出、保湿ジェル等による口腔乾燥予防。
...
①付箋で下書きする（1情報1枚、貼り直しで修正が楽になる）②配置レイアウトを4エリアに分ける（中央上：基本情報、下半分：病態＝原因→症状→治療、左上：社会面、右上：心理面。A3横置きが基本）③矢印は「原因→結果」の因果関係の方向で統一し、単なる時系列では結ばない。治療・看護ケアの矢印はアプローチする対象（症状・問題）に向ける④実線＝現在起きている事実、点線＝今後起こりうる予測・リスクで描き分け、矢印が集中する場所を優先度の高い看護問題(#)とする⑤「だからどうなる？」で生活面・心理面と病態をつなぎ、患者固有のストーリーに個別性を出す⑥完成後は「なぜかというと？」で看護問題から逆にたどり、論理の飛びや孤立したマスがないか最終チェックする。`;
    ...
    // ようにする。単位が書かれているかどうかに関わらず、formatLabValueStringが自動で
    // 単位・基準値を補う（単位が既に書かれている場合も基準値だけは追記する。利用者からの
    // 報告：単位だけ書かれていて基準値が付いていない検査値カードがあった）。
    // ==========================================================================
    ...
    const LAB_KEY_NUM_PATTERNS = [
      ...
      // Ht・PTを追加（LAB_STANDARDSの説明を参照）
      ['Ht', '\\d+\\.?\\d*'], ['PT', '\\d+\\.?\\d*'],
      // ALP・T-Bil・血糖・アミラーゼを追加（LAB_STANDARDSの説明を参照）。「T-Bil」は
    ...
    // 実際の記録では「AST (GOT)」のように項目名と括弧書きの間にスペースが入ることがあり、
    // また紙のカルテのスキャン・OCR由来のテキストでは、全角の丸括弧「（）」の代わりに
    // 全角の波括弧「｛｝」に誤認識されることがある（利用者からのアップロード文書で発覚：
    // 「WBC｛白血球数）」のように開き括弧だけが「｛」になっていた）。開き括弧・閉じ括弧の
    // 組み合わせが一致していない場合（OCR誤認識で片方だけ違う文字になっている場合）にも
    // 対応できるよう、開き括弧・閉じ括弧をそれぞれ独立した候補の集合として扱う。
    //
    // 【修正】この関数の呼び出し元でtext.normalize('NFKC')が行われるため、全角の波括弧
    // 「｛｝」はここに来る時点で半角の「{」「}」に変換されている。しかし元のOCR誤認識では
    // 「｛」と「）」（波括弧と丸括弧）という異なる種類の括弧が対になっており、NFKCはそれぞれを
    // 独立に半角化するため、実際には「{白血球数)」（半角の開き波括弧＋半角の閉じ丸括弧という、
    // 新たに不一致な組み合わせ）になる。この半角「{」「}」が開き・閉じ括弧の候補集合に
    // 含まれていなかったため、正規表現全体が一致せず、「WBC」自体が項目名の行として認識
    // されない不具合があった（利用者からのアップロード文書で発覚）。半角の波括弧も候補に加える。
    ...
    // 項目名ごと取りこぼされていた）。
    // PT（プロトロンビン時間）の秒数表記（例：「12.5秒」）向けに「秒」も単位として追加する
    // （以前は未対応で、PTの値行が「値の行」として認識されず項目名ごと取りこぼされていた）。
    //
    // 【修正】実際にアップロードされた文書では、紙のカルテのスキャン・OCRに由来する単位の
    // 表記ゆれ・誤認識により、この正規表現に一致せず値の行として認識されない実測値が多数
    // 見つかった（利用者からのアップロード文書で発覚：Hb「13.5g/dl」・血糖「89 mg/dl」等の
    // 小文字「dl」表記がAST「150/L」等の単位記号「U」が欠落した表記、RBC「511万/从L」・
    // Plt「28.7万u/L」等の万単位＋「μ」がOCRで「从」「u」に誤認識された表記、Na「140mEa/L」・
    // K「4.4mEa/L」等の「mEq/L」の「q」が「a」に誤認識された表記、WBC「5880/ML.」等の
    // 「μL」の「μ」が脱落し末尾に読点が残った表記）。値そのものは正しく読み取れているにも
    // かかわらず単位の表記ゆれだけで項目名との結合ごと失敗し、項目名の無い意味不明な値だけの
    // カードが残ってしまっていた。
    // - 大文字・小文字を区別しない（'i'フラグ）ことで「dl」「u/l」等の小文字表記に対応する。
    // - 数値の直後に「万」（例：「511万」）が続く場合を許容する。
    // - 「μ」の代わりに「u」「从」「µ」（OCR・機種依存文字の誤認識）が使われる場合を許容する。
    // - 単位記号「U」が欠落した「/L」単独の表記を許容する。
    // - 「mEq/L」の「q」が「a」に誤認識された「mEa/L」表記を許容する。
    // - 「μL」の「μ」が脱落した「mL」単独表記（末尾の読点は末尾の任意の「.」で吸収）を許容する。
    ...
    // 「項目」「種類」「正常・異常」は、「検査項目」「基準値」と同様に血液検査等の表で頻出する
    // サブ見出し（列見出し）だが、これまで未登録だったため、表形式によっては値を伴わない
    // 見出し語だけの意味の無いカードとして残ってしまっていた（利用者からのアップロード文書で
    // 発覚：「項目」「種類」「正常」「異常」（「正常・異常」が「・」で分割されたもの）が
    // それぞれ単独のカードになっていた）。
    const SECTION_HEADER_PASSTHROUGH_KEYS = ["検査項目", "基準値", "項目", "種類", "正常・異常"];
    ...
    const DIAGNOSIS_TAG_HINTS = [
      ...
      // 「胆結石」は「胆石症」の別表記（実際の記録での言い回し）で、「症」を含まないため
      ...
      // 大腿骨近位部骨折（頸部骨折・転子部骨折／人工骨頭置換術BHA・骨接合術）は周術期に
      // DVT/PTE・脱臼予防（禁忌肢位）・術後せん妄・創部/ドレーントラブル・尿路管理など
      // 複数のヘンダーソン項目に関わる合併症リスクが集中するため、上の汎用パターン
      // （骨折→4のみ）よりも広く初期提案する。（参照元：利用者アップロード資料。
      // DEFAULT_NOTEBOOK_CONTENT側にも同資料に基づく詳細な判断基準を追記している）
      { pattern: /大腿骨(?:近位部|頸部|転子部|転子下)?骨折|人工骨頭置換術|人工股関節全置換術|\bBHA\b|\bTHA\b/, tagIds: [1, 3, 4, 9, 10, 14] },
      // 肺炎（市中肺炎CAP・院内肺炎HAP・医療介護関連肺炎NHCAP・誤嚥性肺炎）は呼吸(1)だけでなく、
      // 誤嚥・食欲不振等の食事(2)、発熱の体温(7)、口腔ケア等の清潔(8)、院内感染対策の環境(9)、
      // 嚥下リハ・ワクチン接種等の学び(14)にも関わる合併症・観察項目が集中するため、上の汎用
      // パターン（肺炎→1のみ）よりも広く初期提案する。（参照元：利用者アップロード資料。
      // DEFAULT_NOTEBOOK_CONTENT側にも同資料に基づく詳細な判断基準を追記している）
      { pattern: /誤嚥性肺炎|市中肺炎|院内肺炎|医療[・]?介護関連肺炎|\bCAP\b|\bHAP\b|\bNHCAP\b|\bVAP\b/, tagIds: [1, 2, 5, 7, 8, 9, 14] }
    ];
    ...
    // 「【保険】」「【既往歴】」のように、見出しラベル（FIELD_LABELS）だけが独立した行になって
    // おり、実際の値は次の行に別に書かれている記録形式がある（例：「【保険】\n社会保険」）。
    // この場合、見出し行自体は値を持たないため、これまでは他の内容を持たない括弧書き見出し
    // （「【身長・体重】」等）と同様にそのまま破棄され、直後の値の行（「社会保険」等）が見出しの
    // 文脈を失ったまま無関係な一般カードとして抽出され、見出しラベルに応じたタグ
    // （FIELD_LABEL_DEFAULT_TAGS等）も付与されなくなっていた（利用者からのアップロード文書・
    // スクリーンショットで発覚：「社会保険」がタグ未設定になっていた）。
    // 見出し行を検出した場合、次の行にこの見出しラベルを引き継ぎ、直後の1行だけをその見出し
    // 付きカードとして扱う（呼び出し側のpendingFieldLabelKeys参照）。
    //
    ...
    function detectFieldLabelHeadingOnlyLine(rawLine) {
      ...
      // 単一の見出し語が括弧（開き・閉じの不一致含む）で囲まれているだけの行
      ...
      // 複数の見出し語が「・」でつながれ、末尾に括弧の残骸だけが残っている行
      if (trimmed.includes('・')) {
    ...
    function switchAdminTab(tab) {
      ...
      document.getElementById('admin-tab-btn-trends').classList.toggle('active', isTrends);
      ...
      document.getElementById('admin-panel-trends').classList.toggle('hidden', !isTrends);
      document.getElementById('admin-panel-trends').classList.toggle('flex', isTrends);
...
<body>${bodyHtml}</body>
    ...
    function detectMultipleHendersonTags(text) {
      ...
      if (/「[^」]{1,300}」/.test(text)) tags.add(10);
